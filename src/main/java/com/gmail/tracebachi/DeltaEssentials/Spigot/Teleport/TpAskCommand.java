...
public class TpAskCommand implements TabExecutor, Registerable {
    private static final String COMMAND_NAME = "tpa";
    private static final String COMMAND_USAGE = "/tpa <name>";
    private static final String COMMAND_PERM = "DeltaEss.Tp.MeToOther.Ask";
    ...
    public TpAskCommand(
                        DeltaEssentialsPlugin plugin, TpAskManager tpAskManager, MessageFormatMap formatMap,
                        SockExchangeApi api) {
        Preconditions.checkNotNull(plugin, "plugin");
        Preconditions.checkNotNull(tpAskManager, "tpHereManager");
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (args.length < 1) {
            sender.sendMessage(formatMap.format(DeltaEssentialsConstants.FormatNames.USAGE, COMMAND_USAGE));
            return true;
        }

        if (!(sender instanceof Player)) {
            sender.sendMessage(formatMap.format(DeltaEssentialsConstants.FormatNames.PLAYER_ONLY_COMMAND, COMMAND_NAME));
            return true;
        }

        if (!sender.hasPermission(COMMAND_PERM)) {
            sender.sendMessage(formatMap.format(DeltaEssentialsConstants.FormatNames.NO_PERM, COMMAND_PERM));
            return true;
        }
        ...
        if (playerFile == null) {
            sender.sendMessage(formatMap.format(DeltaEssentialsConstants.FormatNames.PLAYER_FILE_NOT_LOADED));
            return true;
        }

        boolean ignoreBlocking = sender.hasPermission(DeltaEssentialsConstants.Permissions.TP_IGNORE_BLOCKING_PERM);
        boolean ignoreVanish = sender.hasPermission(DeltaEssentialsConstants.Permissions.TP_IGNORE_VANISH_PERM);

        tpAskManager.tpAsk((Player) sender, args[0], ignoreBlocking, ignoreVanish);
        return true;
    }
}
